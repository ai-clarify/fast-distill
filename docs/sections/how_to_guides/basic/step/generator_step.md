...
The [`GeneratorStep`][fastdistill.steps.GeneratorStep] is a subclass of [`Step`][fastdistill.steps.Step] that is intended to be used as the first step within a [`Pipeline`][fastdistill.pipeline.Pipeline], because it doesn't require input and generates data that can be used by other steps. Alternatively, it can also be used as a standalone.
...
if TYPE_CHECKING:
    from fastdistill.typing import StepColumns, GeneratorStepOutput
...
We can define a custom generator step by creating a new subclass of the [`GeneratorStep`][fastdistill.steps.GeneratorStep] and defining the following:
...
=== "Using the `@step` decorator"
    ...
    if TYPE_CHECKING:
        from fastdistill.typing import GeneratorStepOutput
