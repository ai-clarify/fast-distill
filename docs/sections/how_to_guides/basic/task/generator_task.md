...
The [`GeneratorTask`][fastdistill.steps.tasks.GeneratorTask] is a custom implementation of a [`Task`][fastdistill.steps.tasks.Task] based on the [`GeneratorStep`][fastdistill.steps.GeneratorStep]. As with a [`Task`][fastdistill.steps.tasks.Task], it is normally used within a [`Pipeline`][fastdistill.pipeline.Pipeline] but can also be used standalone.
...
from fastdistill.steps.tasks.base import GeneratorTask
from fastdistill.typing import ChatType, GeneratorOutput
...
!!! NOTE
    Most of the times you would need to override the default `process` method, as it's suited for the standard [`Task`][fastdistill.steps.tasks.Task] and not for the [`GeneratorTask`][fastdistill.steps.tasks.GeneratorTask]. But within the context of the `process` function you can freely use the `llm` to generate data in any way.
...
We can define a custom generator task by creating a new subclass of the [`GeneratorTask`][fastdistill.steps.tasks.Task] and defining the following:

- `process`: is a method that generates the data based on the [`LLM`][fastdistill.models.llms.LLM] and the `instruction` provided within the class instance, and returns a dictionary with the output data formatted as needed i.e. with the values for the columns in `outputs`. Note that the `inputs` argument is not allowed in this function since this is a [`GeneratorTask`][fastdistill.steps.tasks.GeneratorTask]. The signature only expects the `offset` argument, which is used to keep track of the current iteration in the generator.
