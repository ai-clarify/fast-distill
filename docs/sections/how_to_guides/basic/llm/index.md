...
LLM subclasses are designed to be used within a [Task][fastdistill.steps.tasks.Task], but they can also be used standalone.
...
!!! Tip "New in version 1.5.0"
    ...
    This data will be moved by the corresponding `Task` during the pipeline processing and moved to `fastdistill_metadata` so we can operate on this data if we want, like for example computing the number of tokens per dataset.
...
# FastDistillOfflineBatchGenerationNotFinishedException: Batch generation with jobs_ids=('batch_OGB4VjKpu2ay9nz3iiFJxt5H',) is not finished
...
2. The first call to `generate_outputs` will send the inputs to the LLM-as-a-service and return a `FastDistillOfflineBatchGenerationNotFinishedException` since the outputs are not ready yet.
...
4. The second call or subsequent calls to `generate_outputs` will return the outputs if they are ready or raise a `FastDistillOfflineBatchGenerationNotFinishedException` if they are not ready yet.
...
Pass the LLM as an argument to the [`Task`][fastdistill.steps.tasks.Task], and the task will handle the rest.
...
#   'fastdistill_metadata': {'raw_output_text_generation': 'The capital of Spain is Madrid.',
...
!!! Note
    As mentioned in *Working with LLMs* section, the generation of an LLM is automatically moved to `fastdistill_metadata` to avoid interference with the common workflow, so the addition of the `statistics` it's an extra component available for the user, but nothing has to be changed in the
    defined pipelines.
...
To create custom LLMs, subclass either [`LLM`][fastdistill.models.llms.LLM] for synchronous or [`AsyncLLM`][fastdistill.models.llms.AsyncLLM] for asynchronous LLMs. Implement the following methods:
...
* (optional) `get_last_hidden_state`: is a method that will take a list of prompts and return a list of hidden states. This method is optional and will be used by some tasks such as the [`GenerateEmbeddings`][fastdistill.steps.tasks.GenerateEmbeddings] task.
...
=== "Custom AsyncLLM"
    ...
    from fastdistill.models import AsyncLLM
    from fastdistill.typing import GenerateOutput, HiddenState
    from fastdistill.typing import ChatType
...
!!! Warning
    Additional LLMs created in `fastdistill` will have to take into account how the `statistics` are generated to properly include them in the LLM output.

## Available LLMs

[Our LLM gallery](../../../../components-gallery/llms/index.md) shows a list of the available LLMs that can be used within the `fastdistill` library.
