...
Being able to export the generated synthetic datasets to Argilla, is a core feature within `fastdistill`. We believe in the potential of synthetic data, but without removing the impact a human annotator or group of annotators can bring. So on, the Argilla integration makes it straightforward to push a dataset to Argilla while the [`Pipeline`][fastdistill.pipeline.Pipeline] is running, to be able to follow along the generation process in Argilla's UI, as well as annotating the records on the fly. One can include a [`Step`][fastdistill.steps.Step] within the [`Pipeline`][fastdistill.pipeline.Pipeline] to easily export the datasets to Argilla with a pre-defined configuration, suiting the annotation purposes.
...
For text generation scenarios, i.e. when the [`Pipeline`][fastdistill.pipeline.Pipeline] contains a single [`TextGeneration`][fastdistill.steps.tasks.TextGeneration] step, we have designed the task [`TextGenerationToArgilla`][fastdistill.steps.TextGenerationToArgilla], which will seamlessly push the generated data to Argilla, and allow the annotator to review the records.

The dataset will be pushed with the following configuration:

- **Fields**: `instruction` and `generation`, both being fields of type `argilla.TextField`, plus the automatically generated `id` for the given `instruction` to be able to search for other records with the same `instruction` in the dataset. The field `instruction` must always be a string, while the field `generation` can either be a single string or a list of strings (useful when there are multiple parent nodes of type [`TextGeneration`][fastdistill.steps.tasks.TextGeneration]); even though each record will always contain at most one `instruction`-`generation` pair.
...
!!! NOTE
    The [`TextGenerationToArgilla`][fastdistill.steps.TextGenerationToArgilla] step will only work as is if the [`Pipeline`][fastdistill.pipeline.Pipeline] contains one or multiple [`TextGeneration`][fastdistill.steps.tasks.TextGeneration] steps, or if the columns `instruction` and `generation` are available within the batch data. Otherwise, the variable `input_mappings` will need to be set so that either both or one of `instruction` and `generation` are mapped to one of the existing columns in the batch data.

```python
from fastdistill.models import OpenAILLM
from fastdistill.steps import LoadDataFromDicts, TextGenerationToArgilla
...
For preference scenarios, i.e. when the [`Pipeline`][fastdistill.pipeline.Pipeline] contains multiple [`TextGeneration`][fastdistill.steps.tasks.TextGeneration] steps, we have designed the task [`PreferenceToArgilla`][fastdistill.steps.PreferenceToArgilla], which will seamlessly push the generated data to Argilla, and allow the annotator to review the records.

The dataset will be pushed with the following configuration:

- **Fields**: `instruction` and `generations`, both being fields of type `argilla.TextField`, plus the automatically generated `id` for the given `instruction` to be able to search for other records with the same `instruction` in the dataset. The field `instruction` must always be a string, while the field `generations` must be a list of strings, containing the generated texts for the given `instruction` so that at least there are two generations to compare. Other than that, the number of `generation` fields within each record in Argilla will be defined by the value of the variable `num_generations` to be provided in the [`PreferenceToArgilla`][fastdistill.steps.PreferenceToArgilla] step.
...
!!! NOTE
    The [`PreferenceToArgilla`][fastdistill.steps.PreferenceToArgilla] step will only work if the [`Pipeline`][fastdistill.pipeline.Pipeline] contains multiple [`TextGeneration`][fastdistill.steps.tasks.TextGeneration] steps, or if the columns `instruction` and `generations` are available within the batch data. Otherwise, the variable `input_mappings` will need to be set so that either both or one of `instruction` and `generations` are mapped to one of the existing columns in the batch data.

!!! NOTE
    Additionally, if the [`Pipeline`][fastdistill.pipeline.Pipeline] contains an [`UltraFeedback`][fastdistill.steps.tasks.UltraFeedback] step, the `ratings` and `rationales` will also be available and be automatically injected as suggestions to the existing dataset.

```python
from fastdistill.models import OpenAILLM
from fastdistill.steps import LoadDataFromDicts, PreferenceToArgilla
from fastdistill.steps.tasks import TextGeneration
