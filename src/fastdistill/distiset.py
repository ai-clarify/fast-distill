...
from fastdistill.utils.card.dataset_card import (
    FastDistillDatasetCard,
    size_categories_parser,
)
from fastdistill.utils.docstring import get_bibtex, parse_google_docstring
from fastdistill.utils.files import list_files_in_dir
from fastdistill.utils.huggingface import get_hf_token
...
class Distiset(dict):
    ...
    def _get_card(
        ...
        card = FastDistillDatasetCard.from_template(
...
def create_distiset(  # noqa: C901
    ...
    Args:
        ...
        enable_metadata: Whether to include the fastdistill metadata column in the dataset or not.
            Defaults to `False`.
    ...
    Examples:
        ```python
        from pathlib import Path
        distiset = create_distiset(Path.home() / ".cache/fastdistill/pipelines/path-to-pipe-hashname")
        ```
    """
    from fastdistill.constants import FASTDISTILL_METADATA_KEY

    logger = logging.getLogger("fastdistill.distiset")
    ...
    for file in steps_outputs_dir.iterdir():
        ...
        if files:
            try:
                ...
                if not enable_metadata and FASTDISTILL_METADATA_KEY in ds.column_names:
                    ds = ds.remove_columns(FASTDISTILL_METADATA_KEY)
