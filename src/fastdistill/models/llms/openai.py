...
from fastdistill.models.llms.utils import prepare_output
...
class OpenAILLM(OpenAIBaseClient, AsyncLLM):
    ...
    Attributes:
        ...
        structured_output: a dictionary containing the structured output configuration configuration
            ...
            `InstructorStructuredOutputType` from `fastdistill.steps.tasks.structured_outputs.instructor`.
    ...
    def offline_batch_generate(
        ...
        **kwargs: Any,
        ...
        Raises:
            FastDistillOfflineBatchGenerationNotFinishedException: if the batch generation
                is not finished yet.
    ...
    def _create_batch_api_job(
        ...
        metadata = {"description": "fastdistill"}

        if fastdistill_pipeline_name := envs.FASTDISTILL_PIPELINE_NAME:
            metadata["fastdistill_pipeline_name"] = fastdistill_pipeline_name

        if fastdistill_pipeline_cache_id := envs.FASTDISTILL_PIPELINE_CACHE_ID:
            metadata["fastdistill_pipeline_cache_id"] = fastdistill_pipeline_cache_id
    ...
    def _create_batch_files(
        ...
        for file_no, buffer in enumerate(
            ...
            try:
                # TODO: add fastdistill pipeline name and id
    ...
    def _name_for_openai_files(self, file_no: int) -> str:
        if (
            envs.FASTDISTILL_PIPELINE_NAME is None
            or envs.FASTDISTILL_PIPELINE_CACHE_ID is None
        ):
            return f"fastdistill-pipeline-fileno-{file_no}.jsonl"

        return f"fastdistill-pipeline-{envs.FASTDISTILL_PIPELINE_NAME}-{envs.FASTDISTILL_PIPELINE_CACHE_ID}-fileno-{file_no}.jsonl"
