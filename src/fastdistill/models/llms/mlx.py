...
class MlxLLM(LLM, MagpieChatTemplateMixin):
    ...
    Examples:
        ...
        ```python
        from fastdistill.models.llms import MlxLLM
    ...
    structured_output: Optional[RuntimeParameter[InstructorStructuredOutputType]] = (
        Field(
            default=None,
            description="Structured output configuration using instructor.",
        )
    )
    ...
    def load(self) -> None:
        ...
        try:
            ...
            raise ImportError(
                "MLX is not installed. Please install it using `pip install 'fastdistill[mlx]'`."
            ) from ie
