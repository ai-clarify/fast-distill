...
class RayPipeline(BasePipeline):
    ...
    def run(
        ...
        self._log_queue = self.QueueClass(
            actor_options={"name": f"fastdistill-{self.name}-log-queue"}
        )
        ...
        self._output_queue = self.QueueClass(
            actor_options={"name": f"fastdistill-{self.name}-output-queue"}
        )
        self._load_queue = self.QueueClass(
            actor_options={"name": f"fastdistill-{self.name}-load-queue"}
        )
    ...
    def _init_ray(self) -> None:
        ...
        try:
            ...
            raise ImportError(
                "ray is not installed. Please install it using `pip install 'fastdistill[ray]'`."
            ) from ie
    ...
    def _create_step_input_queue(self, step_name: str) -> "Queue[Any]":
        ...
        input_queue = self.QueueClass(
            actor_options={"name": f"fastdistill-{self.name}-input-queue-{step_name}"}
        )
