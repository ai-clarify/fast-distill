...
from fastdistill import __version__, constants, envs
from fastdistill.distiset import create_distiset
from fastdistill.errors import FastDistillUserError
from fastdistill.mixins.requirements import RequirementsMixin
...
from fastdistill.steps.base import GeneratorStep, _Step
...
from fastdistill.utils.notebook import in_notebook
from fastdistill.utils.serialization import (
    _Serializable,
    read_json,
)
from fastdistill.utils.typing_ import (
...
if TYPE_CHECKING:
    ...
    from fastdistill.pipeline.routing_batch_function import RoutingBatchFunction
    from fastdistill.steps.base import Step
    from fastdistill.typing import (
        InputDataset,
        LoadGroups,
...
class BasePipeline(ABC, RequirementsMixin, _Serializable):
    """Base class for a `fastdistill` pipeline.
    ...
    def __init__(
        ...
        Args:
            ...
            enable_metadata: Whether to include the fastdistill metadata column for the pipeline
                in the final `Distiset`. It contains metadata used by fastdistill, for example
        ...
        elif env_cache_dir := envs.FASTDISTILL_CACHE_DIR:
            self._cache_dir = Path(env_cache_dir)
        else:
            self._cache_dir = constants.PIPELINES_CACHE_DIR

        self._logger = logging.getLogger("fastdistill.pipeline")
    ...
    def _validate_load_groups(self, load_groups: List[List[Any]]) -> None:  # noqa: C901
        ...
        Raises:
            FastDistillUserError: if something is not OK when checking the load groups.
        """
    ...
    def dump(self, **kwargs: Any) -> Dict[str, Any]:
        return {
            "fastdistill": {"version": __version__},
            "pipeline": {
                "name": self.name,
