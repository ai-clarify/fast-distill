...
def merge_fastdistill_metadata(
    *output_dicts: Dict[str, Any],
) -> Union[Dict[str, Any], List[Dict[str, Any]]]:
    """
    Merge the `FASTDISTILL_METADATA_KEY` from multiple output dictionaries. `FASTDISTILL_METADATA_KEY`
    can be either a dictionary containing metadata keys or a list containing dictionaries
    of metadata keys.

    Args:
        *output_dicts: Variable number of dictionaries or lists containing fastdistill metadata.

    Returns:
        A merged dictionary or list containing all the fastdistill metadata.
    """
    merged_metadata = defaultdict(list)

    for output_dict in output_dicts:
        metadata = output_dict.get(FASTDISTILL_METADATA_KEY, {})
        # If `fastdistill_metadata_key` is a `list` then it contains dictionaries with
    ...
    *inputs: "StepInput",
    ...
    for dicts_at_index in zip(*inputs):
        ...
        for d in dicts_at_index:
            ...
            if FASTDISTILL_METADATA_KEY in d:
                metadata_dicts.append(
                    {FASTDISTILL_METADATA_KEY: d[FASTDISTILL_METADATA_KEY]}
                )
            # Iterate over key-value pairs in each dict
            for key, value in d.items():
                if key == FASTDISTILL_METADATA_KEY:
                    continue
