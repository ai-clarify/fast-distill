...
from fastdistill.errors import FastDistillTypeError, FastDistillUserError
from fastdistill.mixins.requirements import RequirementsMixin
from fastdistill.mixins.runtime_parameters import (
    RuntimeParameter,
    RuntimeParametersMixin,
)
from fastdistill.mixins.signature import SignatureMixin
from fastdistill.utils.serialization import _Serializable, write_json
...
_STEP_INPUT_ANNOTATION = "fastdistill_step_input"
...
extra metadata that allows `fastdistill` to perform validations over the `process` step
...
class _Step(
    ...
    the output of the previous steps. In order to make `fastdistill` know where the outputs
    ...
    def model_post_init(self, __context: Any) -> None:
        from fastdistill.pipeline.base import _GlobalPipelineManager
        ...
        if self.pipeline is None:
            _logger = logging.getLogger(f"fastdistill.step.{self.name}")
            _logger.warning(
    ...
    def load(self) -> None:
        ...
        """
        self._logger = logging.getLogger(f"fastdistill.step.{self.name}")
