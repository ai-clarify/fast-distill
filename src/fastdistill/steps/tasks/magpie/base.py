...
from fastdistill.utils.dicts import merge_dicts
    ...
    def _generate_with_pre_query_template(
        ...
        for input, output, stats in zip(inputs, outputs, statistics):
            ...
            if not self.only_instruction:
                generation["fastdistill_metadata"] = {f"statistics_{self.name}": stats}
...
class Magpie(Task, MagpieBase):
    ...
    Examples:
        ...
        ```python
        from fastdistill.models import TransformersLLM
        from fastdistill.steps.tasks import Magpie
