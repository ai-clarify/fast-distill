...
from fastdistill.steps.tasks.evol_quality.utils import MUTATION_TEMPLATES
...
class EvolQuality(Task):
    ...
    Examples:
        ...
        ```python
        from fastdistill.steps.tasks import EvolQuality
    ...
    def process(self, inputs: StepInput) -> "StepOutput":  # type: ignore
        ...
        for input, response in zip(inputs, responses):
            ...
            input.update(
                {"fastdistill_metadata": {f"statistics_{self.name}": statistics}}
            )
        yield inputs
