...
from fastdistill.utils.dicts import group_dicts
...
class _Task(_Step, ABC):
    ...
    Attributes:
        ...
        add_raw_output: whether to include a field with the raw output of the LLM in the
            `fastdistill_metadata` field of the output. Can be helpful to not loose data
    ...
    add_raw_output: RuntimeParameter[bool] = Field(
        ...
        description=(
            ...
            " of the `fastdistill_metadata` dictionary output column"
        ),
    )
    add_raw_input: RuntimeParameter[bool] = Field(
        ...
        description=(
            ...
            " of the `fastdistill_metadata` dictionary column"
        ),
    )
    ...
    def _format_outputs(
        ...
        for (output, stats, extra), input in zip(
            ...
            try:
                # Extract the generations, and move the statistics to the fastdistill_metadata,
    ...
    def _output_on_failure(
        ...
        a new field `fastdistill_meta` with the raw output of the LLM.
        """
    ...
    def _create_metadata(
        ...
        """
        meta = output.get(FASTDISTILL_METADATA_KEY, {})
        ...
        if meta:
            output[FASTDISTILL_METADATA_KEY] = meta
    ...
    def print(self, sample_input: Optional["ChatType"] = None) -> None:
        ...
        Examples:
            ...
            ```python
            from fastdistill.steps.tasks import URIAL
            from fastdistill.models.llms.huggingface import InferenceEndpointsLLM
