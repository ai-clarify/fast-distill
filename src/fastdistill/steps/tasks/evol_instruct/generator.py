...
from fastdistill.steps.tasks.evol_instruct.utils import GENERATION_MUTATION_TEMPLATES
from fastdistill.utils.lists import flatten_responses

if TYPE_CHECKING:
    from fastdistill.typing import ChatType, GeneratorStepOutput, LLMStatistics
...
class EvolInstructGenerator(GeneratorTask):
    ...
    Examples:
        ...
        ```python
        from fastdistill.steps.tasks import EvolInstructGenerator
    ...
    def process(self, offset: int = 0) -> "GeneratorStepOutput":  # NOQA: C901, type: ignore
        ...
        while len(instructions) < self.num_instructions:
            ...
            if not self.generate_answers and len(instructions[-mutation_no:]) > 0:
                ...
                for mutated_instruction in instructions[-mutation_no:]:
                    ...
                    mutated_instruction["fastdistill_metadata"] = {
        ...
        if self.generate_answers:
            ...
            for instruction, answer in zip(instructions, answers):
                ...
                formatted_output["fastdistill_metadata"] = {
