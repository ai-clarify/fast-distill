...
class WriteScoreAgreementReport(GlobalStep):
    ...
    def _agreement(self, errors: Iterable[float]) -> dict:
        ...
        for eps in self.agreement_epsilons:
            ...
            metrics[key] = sum(1 for err in errors_list if err <= eps) / len(errors_list)
        return metrics
    ...
    def process(self, inputs: StepInput):  # type: ignore[override]
        ...
        if teacher_scores:
            ...
            if self.pass_threshold is not None:
                teacher_pass = [score >= self.pass_threshold for score in teacher_scores]
                student_pass = [score >= self.pass_threshold for score in student_scores]
                agree = sum(
                    1
        ...
        output_path = (
            Path(self.output_dir) / str(self.stage) / "score_agreement.json"
        )
